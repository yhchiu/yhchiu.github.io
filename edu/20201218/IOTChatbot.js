...
var adafruitUsername = "XXXXXXXXXXXXXX";
var adafruitKey = "XXXXXXXXXXXXXX";
var adafruitPowerFeed = "fan";
var adafruitTempFeed = "temp-humi";
...
function doPost(e) {
  ...
  var returnText = "";
  var hasKeywordTemp = false;
  var hasKeywordOpen = false;
  var hasKeywordClose = false;

  if (typeof keyWordsTemp === 'undefined') {
    var keyWordsTemp = ["溫度", "幾度", "濕度"];
  }
  else {
    keyWordsTemp = keyWordsTemp.concat(["溫度", "幾度", "濕度"]);
  }

  if (typeof keyWordsOpen === 'undefined') {
    var keyWordsOpen = ["開", "啟動", "open"];
  }
  else {
    keyWordsOpen = keyWordsOpen.concat(["開", "啟動", "open"]);
  }

  if (typeof keyWordsClose === 'undefined') {
    var keyWordsClose = ["關", "close"];
  }
  else {
    keyWordsClose = keyWordsClose.concat(["關", "close"]);
  }

  if (userMessage) {
    for (var i = 0; i < keyWordsTemp.length; i++) {
      if (userMessage.indexOf(keyWordsTemp[i]) !== -1) {
        hasKeywordTemp = true;
        break;
      }
      else if (userMessage.indexOf(keyWordsOpen[i]) !== -1) {
        hasKeywordOpen = true;
        break;
      }
      else if (userMessage.indexOf(keyWordsClose[i]) !== -1) {
        hasKeywordClose = true;
        break;
      }
    }
  }
  ...
  if (hasKeywordTemp) {
    var url = 'https://io.adafruit.com/api/v2/'+adafruitUsername.trim()+'/feeds/'+adafruitTempFeed.trim()+'/data';
    var response = UrlFetchApp.fetch(url, {
      'headers': {
        'Content-Type': 'application/json; charset=UTF-8',
        'X-AIO-Key': adafruitKey.trim(),
      },
      'method': 'get',
    });
    var data = JSON.parse(response.getContentText());
    returnText = new Date(data[0].created_at).toLocaleString("zh-TW", {timeZone: "Asia/Taipei"}) + " 的溫濕度是 " + data[0].value;
  }
  else if (hasKeywordOpen) {
    returnText = '正在開啟...';

    var url = 'https://io.adafruit.com/api/v2/'+adafruitUsername.trim()+'/feeds/'+adafruitPowerFeed.trim()+'/data';
